...
try:
    from src.features.audio_features import extract_features_batch
...
def main():
    ...
    human_files = glob.glob(str(RAW_DATA_DIR / "human" / "*.wav")) + glob.glob(str(RAW_DATA_DIR / "human" / "*.m4a"))
    ai_files = glob.glob(str(RAW_DATA_DIR / "ai" / "*.wav")) + glob.glob(str(RAW_DATA_DIR / "ai" / "*.m4a"))
    
    print(f"Found {len(human_files)} human, {len(ai_files)} AI samples")
    
    if len(human_files) == 0 or len(ai_files) == 0:
        print("❌ No training data found!")
        print("Add WAV files to data/raw/human/ and data/raw/ai/")
        return False
